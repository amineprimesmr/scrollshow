...
export async function GET(request:Request){
  ...
  for(const job of jobs){if(Date.now()>deadline-45000)break;const u=data.users.find(u=>u.id===job.userId&&!u.deletionPendingAt&&hasStudioAccess(u.plan));if(!u)continue;const project=job.projectId?findProject(data,u.id,job.projectId):null;if(!project||!u.emailVerifiedAt)continue;try{await advanceResearch(withProject(publicUser(u),project),job.id);steps++;}catch{/* A failed job cannot starve other workspaces. */}}
