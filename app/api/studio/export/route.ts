...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      researchJobs: (data.researchJobs || []).filter(r => r.userId === session.id).map(({ lease, ...job }) => job),
      formatStudies: (data.formatStudies || []).filter(s => s.userId === session.id),
      publicationText: (data.publicationText || []).filter(s => s.userId === session.id),
