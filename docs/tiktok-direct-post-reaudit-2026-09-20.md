...
## Nouveau parcours intégral demandé

- L'accès local aux fichiers a finalement été activé par Amine. Les deux premiers MP4 ont été téléversés et le brouillon enregistré, sans soumission. Ils doivent maintenant être remplacés par une seule vidéo incluant un nouveau compte et l'onboarding, à la demande d'Amine.
- `fe5aac9` : l'étape agent de l'onboarding précise désormais que les agents préparent des brouillons et que le créateur vérifie les contenus et options dans le studio.
- `123069f` : après autorisation explicite d'Amine, les changements existants de codes promo Stripe ont été vérifiés, commités séparément et déployés. Les sept tests ciblés Stripe/TikTok et le typecheck passent. Déploiement de production `dpl_2VUGXFtQkctKSNhWWgcjQwFhmFov` Ready.
- Coupon Stripe à usage unique préparé pour une remise de 100 % sur l'offre à vie uniquement. Aucune souscription, aucun débit ni activation artificielle du plan à ce stade.
- Nouveau tournage démarré dans une fenêtre Chrome dédiée avec barre d'adresse visible. Landing, pricing, conditions, confidentialité et support parcourus. Inscription arrivée à la création du mot de passe ; compte pas encore créé. La vidéo intégrale reste à terminer et à vérifier avant remplacement des anciens fichiers.
- Le nouveau compte a été créé puis confirmé le 20 septembre à 16:16:28 UTC. Le lien utilisé une seconde fois a logiquement été refusé ; diagnostic confirmé par lecture des seuls champs d'état du compte. Reconnexion normale réussie, sans modification du mot de passe ni contournement de vérification.
- Onboarding réel parcouru : analyse de scrollshow.io, prénom Amine, entreprise ScrollShow, étapes facultatives statistiques publiques/agent visibles puis passées, source Autre. Commande Stripe réelle terminée, remise de 99 EUR, total 0 EUR, aucun PaymentIntent et aucun renouvellement.
- Défaut découvert : `applyLifetime` exigeait un PaymentIntent et un total de 99 EUR même avec un code promo accepté par Checkout. `f315958` autorise les commandes terminées remisées uniquement lorsque le sous-total reste de 99 EUR et que la remise Stripe explique exactement la différence. Les commandes gratuites n'ont pas de PaymentIntent ; leur statut peut être `paid` (observé ici) ou `no_payment_required`. Tests négatifs (compte, devise, prix, remise, état) et remboursements conservés. 36 tests ciblés et typecheck réussis ; test de la variante réelle `paid` sans PaymentIntent ajouté et réussi.
- Après déploiement Ready de `f315958` (`dpl_BiunjAL9jANdFuXoc9xKuuM1Tra6`), le retour normal `/pricing/success` réconcilie la même commande Stripe et ouvre le studio. L'accès Lifetime est visible dans les réglages.
- Nouveau compte : onboarding réel terminé, studio passé en anglais, OAuth TikTok à cinq scopes réussi avec @mannyprcs. Overview, Recherche, Bibliothèque et Calendrier visités. Les anciennes couvertures restent défaillantes ; ne pas présenter la vidéo comme exempte de défauts.
- Nouveau carrousel original : `Make room for one good idea`, deux slides texte blanc/fond uni. Only me choisi à l'écran, commentaires ON, musique OFF, commercial ON sans sous-option montré bloqué, puis Your brand ON. Publication réelle réussie : post `bbfcf32e-e65e-47e8-b9d6-9aa999aa77c5`, `status=published`, `publishState=PUBLISH_COMPLETE`, approbation du studio à 16:30:03 UTC. Résultat privé et promotionnel visible sur https://www.tiktok.com/@mannyprcs/photo/7687655604231359766.
- Les captures du parcours intégral sont assemblées en une seule copie de relecture `03-full-flow-review.mp4`. Ce n'est pas une prise continue : saisie du mot de passe et confirmation email hors caméra, email montré après validation, interruption pour corriger Stripe, détail des permissions refilmé ensuite. Des cartons l'indiquent explicitement ; aucun état de l'app n'est simulé. Cette limitation doit être relue avant toute soumission.
