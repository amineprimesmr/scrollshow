...
import { restoreBusinessBackup, assertBusinessRestoreTargetEmpty } from "../lib/business-backup";
async function main() {
...
if (manifest?.business && !process.argv.includes("--confirm-business-schema-migrated")) throw new Error("Apply business schema migrations to the empty target, then add --confirm-business-schema-migrated. Business connections/keys/links restore disabled.");
...
try {
  await sql.begin(async tx => {
    ...
    if (manifest?.business) await restoreBusinessBackup(tx, manifest.business, process.env.BACKUP_ENCRYPTION_KEY!, readPart);
    else await assertBusinessRestoreTargetEmpty(tx, true);
    if (manifest) await restoreBackupBundle(manifest, process.env.BACKUP_ENCRYPTION_KEY!, readPart, targetStorage);
  ...
  console.log(`Encrypted snapshot, ${fileCount} media files and ${manifest?.business?.rows ?? 0} business rows restored and verified. Access, API keys and publishing are quarantined. Reconcile external billing, account deletions and publication status before clearing restoreReviewRequired.`);
  if (!manifest?.business) console.warn("Legacy archive: business analytics SQL tables were not included and were not restored.");
