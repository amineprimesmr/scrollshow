...
export async function advanceResearch(user:SessionUser,id:string) {
  ...
  try {
    if(task.kind==="search")result={kind:"search",...await withMetricsUser({userId:user.id},()=>searchPhotos(task.keyword,task.cursor,task.searchId))};
    else {
      ...
      const [page,profile]=await Promise.all([withMetricsUser({userId:user.id},()=>fetchAccountVideoPage(task.candidate.handle,task.candidate.cursor??0)),task.candidate.pages?Promise.resolve(null):fetchTikTokProfile(task.candidate.handle).catch(()=>null)]);
