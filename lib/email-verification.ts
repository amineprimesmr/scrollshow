...
const updateStore = <T>(fn: Parameters<typeof updateStoreSlice<T>>[1]) => updateStoreSlice(["apiKeys", "oauthCodes", "oauthTokens", "oauthUsedRefresh"], fn);
...
export async function deliverVerification(userId: string) {
  ...
  if (item) await sendAccountEmail(item.email, "ScrollShow — Confirme ton adresse", `Ton espace est presque prêt. Confirme ton adresse pour continuer : ${accountLink("/signup?verify=1", item.token)}\nCe lien personnel est valable 24 heures. Si tu n’as pas créé ce compte, ignore simplement cet email.`, "Confirmer mon adresse");
}
...
export async function redeemEmailChange(token: string) {
  return updateStore(data => {
    ...
    user.sessionVersion = (user.sessionVersion || 0) + 1;
    user.googleId = undefined; user.githubId = undefined;
