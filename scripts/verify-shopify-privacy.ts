...
async function main(){
  ...
  finally{try{await deleteUser(`privacy-owner-${suffix}`);}finally{await Promise.all([database().end(),closeBusinessDatabase(),(globalThis as typeof globalThis & {ssShopifyLockSql?:ReturnType<typeof postgres>}).ssShopifyLockSql?.end()]);}}
}
